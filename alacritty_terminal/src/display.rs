...
impl Display {
    ...
    fn get_font<'a>(_font_size: &font::Size, config: &'a Config) -> &'a crate::config::Font {
        // Placeholder for logic that can pick different fonts based on size.
        config.font()
    }
    ...
    pub fn new(config: &Config, options: &cli::Options) -> Result<Display, Error> {
        ...
        let font = Self::get_font(&font_size, config);
        let metrics = GlyphCache::static_metrics(config, font, estimated_dpr as f32)?;
        ...
        let (glyph_cache, cell_width, cell_height) =
            Self::new_glyph_cache(dpr, &mut renderer, Self::get_font(&font_size, config), config)?;
        ...
        Ok(Display {
            ...
            glyph_cache: {
                ...
                map.insert(font_size, (glyph_cache, cell_width, cell_height));
                map
            },
            render_timer,
            tx,
    ...
    pub fn calculate_ufi_info(
        ...
        for i in 1..100 {
            ...
            let font = Self::get_font(&size, config).clone().with_size(size);
                               ...
                               font_size: font::Size) -> Result<&'a mut (GlyphCache, f32, f32), Error>
    {
        ...
        match multicache.entry(font_size) {
            Entry::Vacant(v) => {
                let font = Self::get_font(&font_size, config).clone().with_size(font_size);
    ...
    pub fn draw(&mut self, terminal: &FairMutex<Term>, config: &Config) {
        ...
        let font = Self::get_font(&self.font_size, config);
...

            {
                ...
                self.renderer.with_api(config, &size_info, |mut api| {
                    ...
                    for cell in grid_cells {
                        ...
                        // Draw the cell
                        api.render_cell(cell, glyph_cache, (0.0, 0.0));
                    }
                });
            }
            ...
            for (zoom_idx, zoom) in zooms.iter().enumerate() {
                ...
                self.renderer.with_api(config, &size_info, |mut api| {
                    for cell in &zoom_cells[zoom_idx] {
                        ...
                        // Draw the cell
                        api.render_cell(cell, glyph_cache, text_zoom_offset);
                    }
                });
